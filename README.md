...
CPU/PhysX smoke；asset config 已切换为新 USD，task-level 单环境 CPU/CUDA smoke 已通过。延迟动作、观测、奖励和 PPO/GRU 配置尚未迁移。
...
`SerialLeg-Flat-ClosedChain-v0` 已切换到预生成 USD，可用上述 task smoke 进行 CPU/CUDA 单环境回归；这不代表 RL 动作/观测/奖励/PPO 已迁移完成。
...
1. 恢复 fixed-tendon coupled limits、必要 collision filtering 以及 contact/solver 参数。
2. 实现自定义 6D delayed action term。
3. 对齐 34D actor observation 和 40D critic observation。
4. 迁移 flat rewards、terminations 和 PPO/GRU 配置。

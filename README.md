...
flat 任务 34D/40D 单帧观测、Recovery 任务 138D/168D 五帧历史观测、非跳跃 8D `velocity_height`
command、基础 events/domain randomization、terminations、curriculum、IsaacLab 官方 locomotion rewards，
以及 feed-forward RSL-RL MLP/PPO 配置；flat 的 4 环境最小 PPO update 与 checkpoint save/load round-trip
已通过，Recovery 历史输入的目标规模容量验收仍待完成。
...
`SerialLeg-Recovery-v0` 从 flat 基础任务继承 scene、6D delayed action、8D command 和 curriculum，并使用
Recovery 专属的 feed-forward MLP 输入：当前 8D command 不入历史，policy proprioception 与 critic
privileged proprioception 分别叠 5 帧。控制频率为 50 Hz，因此 5 个样本覆盖最近 80 ms。其余差异包括：
...
Recovery 启用 height-conditioned action default；`0.25 rad` 腿 scale、`10 rad/s` 轮 scale、raw action clip
和 `4–6 ms` FIFO delay 与 flat 保持一致。由于网络输入从 flat 的 34D/40D 改为 138D/168D，旧 flat
checkpoint 的第一层形状不兼容，必须新建 Recovery run，不能直接加载 `model_499.pt`。首次训练前先确认
远端数据盘余量和 checkpoint 外部备份位置。

```bash
OMNI_KIT_ACCEPT_EULA=YES uv run se3rl train --task SerialLeg-Recovery-v0 \
  --envs 4096 --iterations 500 --run-name recovery_history5
```
...
- flat observation mapping：actor 读取 `actor` 34D，critic 读取 `critic` 40D；
- Recovery observation mapping：actor 按顺序读取 `command` 8D + `proprio` 130D，critic 读取
  `command` 8D + `privileged` 160D；历史由 observation manager 按 term-major、帧从旧到新展开，reset
  首帧填满全部 5 个历史槽位；
- actor/critic hidden dims 均为 `[512, 256, 128]`，activation 为 `elu`；
- actor 使用 scalar Gaussian distribution；flat 与 Recovery initial standard deviation 均为 `1.0`；
- actor observation normalization 关闭，保持部署输入合同；critic normalization 开启，以处理原始 wheel contact force 与其他 privileged state 的尺度差异；
- flat 保持单帧；Recovery 只叠 proprioception 历史，当前 command 不叠帧；两者都不引入 recurrent hidden state 或 BPTT；
- rollout 为每环境 24 steps，与现有 command/push curriculum 的 `steps_per_policy_iteration=24` 对齐；
- flat PPO 使用 clipped value loss、clip `0.2`、entropy `0.01`、5 epochs、4 mini-batches、adaptive
  `1e-3` learning rate、`gamma=0.99`、`lam=0.95`、target KL `0.01` 和 max grad norm `1.0`；Recovery
  仅覆盖 entropy `0.00516`、learning rate `3e-4` 和 target KL `0.008`；
...
- `joints.policy_order` 锁定 6D action/observation 顺序；legs/wheels 的 `action_scale=0.25/10.0` 由
...
浏览器仿真工具位于独立 submodule `websim_se3/`，使用项目自有部署合同
`se3_rl_lab.websim.deployment.v1`。Recovery policy 通过 `scripts/rsl_rl/play.py`
导出 ONNX 时会自动写入 138D observation、6D mixed action、时序、电机与资产 fingerprint metadata。

初始化：

```bash
git submodule update --init --recursive websim_se3
```

当前完成 Python model discovery/session server、metadata parser 和 React/Vite shell；MuJoCo WASM、
ONNX Runtime Web、three.js、SerialLeg scene 与 mixed-control runtime 仍在 `websim_se3/.agent-handoff/backlog.md` 跟踪。
